		...
		.then((response) => {
			if (response && response.ok) {
				...
				return Promise.reject(
					new Error('Something went wrong in the request')
				);
			}
		})
		.then((arrayBuffer) => {
			const decodedString = isoTextdecoder.decode(
...
const createFetchPromise = (/** @type configFilenameList */ filename) => {
	return urlToString(configUrlMap.get(filename))
		.then((str) => {
			if (!str) {
				return Promise.reject(
					new Error('Got no url data for ' + filename)
				);
			}
			try {
		...
		switch (deviceInfo.device.type) {
			case 'HmIP-FROLL': { // Shutter actuator - flush mount
				...
				if (homematicDeviceDiv.dataset.hmReadonly === undefined) {
					...
					if (
						homematicDeviceDiv.dataset.hmSafeStateOnly === undefined
					) {
						homematicDeviceDiv.appendChild(
							createButton(
								'Halb',
					...
					addHmMonitoring(
						...
						(valueStr) => {
							...
							if (isNaN(value) || value == 0) {
								...
								setpointDiv.innerText =
									'Sollwert: ' + value.toString() + ' °C';
							}
						}
					);
				}
			...
			case 'HmIP-BSL': { // Switch actuator for brand switches – with Signal Lamp
				...
				addHmMonitoring(
					deviceInfo.selectedDatapoints[4].iseId,
					(valueStr) => {
			...
			case 'HmIP-RCV-50': {
				...
				if (!overrideDatapointTypeArr) {
					homematicDeviceDiv.firstChild.nodeValue =
						'missing datapoint info';
					break;
				}
						...
						addHmMonitoring(
							...
							(valueStr) => {
								...
								if (valueStr === 'true') {
									...
									sysvarButtonFalse.classList.remove(
										'hm-selected'
									);
								} else if (valueStr === 'false') {
									sysvarButtonTrue.classList.remove(
										'hm-selected'
									);
									sysvarButtonFalse.classList.add(
										'hm-selected'
				...
				case '16' /** Value list */:
					{
						...
						for (let index = 0; index < valueList.length; index++) {
							...
							sysvarButton.disabled =
								homematicSysvarDiv.dataset.hmReadonly !==
								undefined;
						...
						addHmMonitoring(
							...
							(valueStr) => {
								...
								oldValue = valueStr;
								let allButtons =
									homematicSysvarDiv.getElementsByClassName(
										'hm-sysvar-value-list'
									);
								for (
									let index = 0;
									...
									if (index.toString() === valueStr) {
										allButtons[index].classList.add(
											'hm-selected'
										);
									} else {
										allButtons[index].classList.remove(
											'hm-selected'
										);
									}
								}
...
function getDeviceInfo(
	...
	return {
		...
		weekProgramm: {
			iseId:
				weekProgramLockDatapointNode?.parentElement.getAttribute(
					'visible'
				) === 'true'
...
let hmFetchNotification = function () {
	if (
		...
		urlToDoc(host + baseXMLAPIpath + 'systemNotification.cgi').then(
			(doc) => {
